...
/*
 * rtk_chip.h - Realtek RTD SoC chip identification
 *
 * Stub for 6.18 porting. The RTD1295 never calls get_rtd_chip_id()
 * at runtime in r8169soc.c (it is only used for RTD139x/16xx/13xx
 * calibration paths guarded by CONFIG_ARCH_RTD139x etc., which we
 * do NOT define). So we only need the header to compile cleanly.
 */
...
static inline int get_rtd_chip_id(void) { return CHIP_ID_RTD1295; }
...
static inline int get_rtd_chip_revision(void) { return RTD_CHIP_A00; }
