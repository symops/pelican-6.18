...
/*
 * IDMAC data transfers never complete (no completion, no timeout either)
 * on this instance of the IP -- force PIO. Confirmed on real Realtek
 * RTD1296 (WD My Cloud Home Duo) hardware: command-only completions work
 * fine over IRQ, but the very first real DMA data phase (CMD8/
 * SEND_EXT_CSD) hangs forever; switching to PIO fixes it outright. See
 * drivers/mmc/host/dw_mmc-rtd129x.c.
 */
#define DW_MMC_QUIRK_NO_DMA                    BIT(2)
